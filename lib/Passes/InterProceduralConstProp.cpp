...
bool propagateStateConstantsWorklist(
  ...
  while (!worklist.empty() && iterations < kMaxIterations) {
    ...
    // Follow musttail edges from the target for constant propagation only
    // (analysis_only=true).  We do NOT rewrite musttail call sites in the
    // target — that would break other callers of the shared function.
    // But we DO need to propagate constants through the target's musttail
    // chain so that downstream dispatch edges are discovered (e.g.,
    // blk_180001cd6 → blk_180001d25 → blk_180001d27).
